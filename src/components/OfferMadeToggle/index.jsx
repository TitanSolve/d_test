...
const OfferMadeToggle = ({ title, madeOffers, onAction, myOwnWalletAddress }) => {
  ...
  useEffect(() => {
    ...
    console.log("OfferMadeToggle-->sellOffer-->", madeOffers);
  }, [madeOffers]);
          ...
          {count > 0 && (
            <div className="space-y-4">
              { sellOffers.map((offer, index) => (
                <OfferMadeCard sellOffer={offer} key={index} onAction={onAction} myWalletAddress={myOwnWalletAddress} />
              ))}
            </div>
          )}
        </motion.div>
